<?php
	...
	if ($tipoPersona == 1){
		...
		if ($result1 = mysql_fetch_assoc($result)){
			...
			$result = mysql_query("select CS.fechaCuota, C.montoCuota, C.idRecargo, CS.idCuota  FROM  persona P,socio S, cuota C, cuota_socio CS where P.cedulaPersona = '$cedulaPersona' AND P.cedulaPersona = S.cedulaPersona AND S.cedulaPersona = CS.cedulaPersona AND CS.idCuota = C.idCuota AND C.tipoCuota = '1' ORDER BY CS.idCuota DESC LIMIT 1");
			if ($result1 = mysql_fetch_assoc($result)){
